...
int main() {
	...
	FILE* ResultPointer = fopen(Result, "w");             //stepLengthAvg pointer
	...
	while (fgets(temp, 11, finaloutput1Pointer))
 	{
		...
		else
		{
			...
			{
				printf("t6(%d)---------",confirm[x]);
				confirm[x]=confirm[x]+1;
				if(confirm[x]==3)		//check if foot steped for 5 frames
				{
					printf("t7");
					printf("\nheyyyyyy (%d)) = %d---------------\n", x, u);
					finall[x][legnum[x]]=u;
	...
	printf("%d, %d, %d, %d",lengths[0],lengths[1],lengths[2],lengths[3]);
	fclose(finaloutput1Pointer);
